...
pauseButton.className = 'header_button pause';
pauseButton.setAttribute('type', 'button');
pauseButton.innerHTML = '<img src="styles/icons/pause.svg" alt="svg">';
pauseButton.addEventListener('click', pause);
 
headerButton.insertAdjacentElement('beforebegin', pauseButton);
...
function generate_task() {
    ...
    for (let i = 1; i < count_items[level - 1]; i++) {
...
function noMoreLeft(arrAnswers) { //проверка остались ли еще подходящие слова
    let ans = true
    document.querySelectorAll('.word').forEach(item => {
